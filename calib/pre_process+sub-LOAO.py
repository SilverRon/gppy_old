...
path_raw    = '/data3/grb/'
#path_raw	= '/data3/paek/factory/test/'
path_gal    = '/data3/IMSNG/IMSNGgalaxies/'
path_mframe = '/data3/paek/factory/master_frames/'

path_c_LOAO = '/data3/IMSNG/'+obs+'/2019/'

path_ref	= '/data3/paek/factory/ref_frames/'+obs
#============================================================
...
def move_calib(obs, path_calib, path_gal):
	...
	for gal in list_gal:
		if gal in list_dir:
			...
			com = 'cp '+path_calib+'*Calib*'+gal+'*.fits '+path_gal+gal+'/'+obs+'/'
			os.system(com);print(com)
			#com = 'cp '+path_calib+'hdCalib*'+gal+'*.fits '+path_gal+gal+'/'+obs+'/'
			#os.system(com);print(com)
		else:
...
for path in new_LOAO:
	...
	for filte in fil_list:
		...
		#------------------------------------------------------------------------------#
			...
			# MASTER FLAT
			order_fil   += 1
			comment = 'Processing\t'+filte+'\t['+str(order_fil)+'/'+str(len(fil_list))+']'  ; print(comment)
...
#        img     = im.split('/')[-1]
	...
	#------------------------------------------------------------
	#	SUBTRACTION FOR SINGLE IMAGE
	#------------------------------------------------------------
	reflist	= glob.glob(path_ref+'/Ref-*.fits')
	callist	= glob.glob(new+'/Calib-*.fits')
	noreflist	= []
	for calpath in callist:
		inim	= os.path.basename(calpath)
		part	= inim.split('-')
		refim	= '-'.join(['Ref', part[1], part[2], part[5]+'.fits'])
		refpath	= path_ref+'/'+refim
		if refpath in reflist:
			gregistering(glob.glob(calpath), refpath)
			os.system('mv ./Calib-*ter.fits '+new)
			hotpants(glob.glob(calpath[:-5]+'_gregister.fits'), refpath)
			#hotpants(glob.glob(inim[:-5]+'_gregister.fits'), refpath)
		else:
			noreflist.append(inim)

	os.system('cp '+new+'/*Calib*.fits '+path_calib)
	os.system('chmod 777 '+path_calib+'*')
	os.system('rm '+new+'/*Calib*.fits')
	os.system('rm '+path_calib+'hcCalib*.fits')
	os.system('rm '+path_calib+'Calib*gregister.fits')
	f		= open(new+'/sub_fail.txt', 'w')
	f.write('#filename\n')
	for inim in noreflist:
		f.write(inim+'\n')
	f.close()

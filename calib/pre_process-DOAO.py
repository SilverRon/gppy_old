...
def change_object_hdu(path) :
    comment = '-'*80+'\n' \
            + 'Change OBJECT Header ...'
    print(comment)
    imlist  = glob.glob(path+'/*object*.fits')
    for img in imlist :
...
new_DOAO    = folder_check(obs)
### CALIBRATION
order       = 0
for path in new_DOAO:
    order   += 1
    new     = path_factory+path.split('/')[-2]
    change_object_hdu(new)
    date    = new.split('/')[-1]    ; date  = ('').join(date.split('_'))
    comment = 'REDUCTION START\t['+str(order)+'/'+str(len(new_DOAO))+'] : '+path; print(comment)
...
#        img     = im.split('/')[-1]
    ...
    os.system('cp -rf '+new+'/ '+path_c_DOAO)
    os.system('rm -rf '+new+'/')
    f       = open(path_log+obs.lower()+'.log', 'a')
    f.write(path+'\n')
    f.close()
    comment = 'REDUCTION DONE\t['+str(order)+'/'+str(len(new_DOAO))+'] : '+path+'\n' \
            + '='*80+'\n'
    print(comment)
#------------------------------------------------------------------------------#
### REMOVE UNNESSARY FILES AND MOVE CALIBRATED FILES
##  LIST FOR CALIB IMAGES
if len(new_DOAO) != 0: move_calib(obs, path_calib, path_gal)

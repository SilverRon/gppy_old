...
path_c_SOAO = '/data3/IMSNG/'+obs+'/'
#------------------------------------------------------------------------------#
### 1. FOLDER CHECK
def folder_check(obs):                                # obs = 'soao'    
    '''
...
def change_filter_hdu(path) :
    comment = '-'*80+'\n' \
            + 'Change FILTER Header ...'
    print(comment)
    imlist  = glob.glob(path+'/*.fits')
    for img in imlist :
        data, hdr   = fits.getdata(img, header = True)
        if  hdr['FILTER']   == 1: hdr['FILTER'] = 'U'
        elif hdr['FILTER']  == 2: hdr['FILTER'] = 'B'
        elif hdr['FILTER']  == 3: hdr['FILTER'] = 'V'
        elif hdr['FILTER']  == 4: hdr['FILTER'] = 'R'
        elif hdr['FILTER']  == 5: hdr['FILTER'] = 'I'
        fits.getheader(img, 0)
        # os.system('rm '+path+img)
        fits.writeto(img, data, hdr, clobber=True)
...
def fnamechange(inim, obs) :
    ...
    if      'U' in fil  : filname   = 'U'
    elif    'B' in fil  : filname   = 'B'
    elif    'V' in fil  : filname   = 'V'
    elif    'R' in fil  : filname   = 'R'
    elif    'I' in fil  : filname   = 'I'
    else                : filname   = 'NA'
...
new_SOAO    = folder_check(obs)
### CALIBRATION
order       = 0
for path in new_SOAO:
    order   += 1
    new     = path_factory+path.split('/')[-2]
    change_filter_hdu(new)
    date    = new.split('/')[-1]    ; date  = ('').join(date.split('_'))
    comment = 'REDUCTION START\t['+str(order)+'/'+str(len(new_SOAO))+'] : '+path; print(comment)
    ...
    obj_list= []
    fil_list= []
    for hdu, fname in images.hdus(imagetyp='object', return_fname=True):
        meta= hdu.header
        fil_list.append(str(meta['FILTER']))
        obj_list.append(str(meta['OBJECT']))
    ...
    for hdu, fname in images.hdus(imagetyp='flat', return_fname=True):
        meta= hdu.header
        band_list.append(str(meta['FILTER']))
...
#        img     = im.split('/')[-1]
    ...
    os.system('cp -rf '+new+'/ '+path_c_SOAO)
    os.system('rm -rf '+new+'/')
    f       = open(path_log+obs.lower()+'.log', 'a')
    f.write(path+'\n')
    f.close()
    comment = 'REDUCTION DONE\t['+str(order)+'/'+str(len(new_SOAO))+'] : '+path+'\n' \
            + '='*80+'\n'
    print(comment)
#------------------------------------------------------------------------------#
### REMOVE UNNESSARY FILES AND MOVE CALIBRATED FILES
##  LIST FOR CALIB IMAGES
if len(new_SOAO) != 0: move_calib(obs, path_calib, path_gal)

...
def LCplot(ax, t0, t, mag, magerr, offset=0, label='Somewhere', marker='o', color='grey', scale='linear', alpha=0.5):
	params_plot	= dict(	x=t-t0,
						y=mag+offset,
						yerr=magerr,
						marker=marker, ms=10, mew=1,
						mec='k', mfc=color, fmt='ko', alpha=0.7,
						capsize=5, capthick=1,
						label=label)
	ax.errorbar(**params_plot)
#------------------------------------------------------------
path_save	= '/home/sonic/SN2018kp'
phot0	= ascii.read(path_save+'/phot4SN2018kp.dat')
...
t0 = 2458142.86257					#	ZTF DISCOVERY
t0_ut = '2018-01-24T08:42:06'
#off		= -0.1
off		= 0.0
...
photR_KHAO	= phot[	(phot['filter']=='R')&
					(phot['obs']=='KHAO_MDFTS')]
#------------------------------------------------------------
photR_MAO	= phot[	(phot['filter']=='R')&
					((phot['obs']=='MAO_SNUCAM') | (phot['obs']=='MAO_FLI'))]
#============================================================
...
params_LCplot_R	= dict(	ax=ax,
						t0=t0,
						t=np.copy(photR_MAO['jd']),
						mag=np.copy(photR_MAO['mag']),
						magerr=np.copy(photR_MAO['magerr']),
						label='MAO R',
						marker='.',
						color='tomato',
						alpha=0.5,
